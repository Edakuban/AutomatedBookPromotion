...
Die globale Buchauswahl liegt unter **Einstellungen → Buchauswahl für den täglichen Flow**. **Zufällig aus allen aktivierten Büchern** lässt Supabase bei jeder neuen Reservierung gleichverteilt eines der geeigneten, übertragenen Bücher wählen. **Nur ein bestimmtes Buch** beschränkt neue Reservierungen auf das ausgewählte Buch, bis die Einstellung wieder geändert wird. Ein bereits offener Entwurf behält seine bisherige Buchauswahl und muss zuerst abgeschlossen oder verworfen werden. Das Formular bearbeitet die vorhandene `promotion_settings`-Konfiguration des eingerichteten Instagram-Kontos; Zugangsdaten und Konto-ID werden nicht an den Browser ausgegeben. Bei einer zwischenzeitlichen Änderung verweigert eine Revisionsprüfung das Überschreiben und verlangt ein erneutes Laden der Seite.
