...
- **Buch → Bucheinstellungen bearbeiten:** Titel, Autor, Zieladresse, Promotion-Vormerkung sowie Buchprofil, Bildprompt-Basis und Caption-Vorgaben bearbeiten und speichern. Unter **Bild-Overlay** eine lokale Schriftart und Titelfarbe wählen; die Seite rendert eine transparente 1024×1024-Vorschau mit dem Buchtitel links oben. Die Vormerkung startet noch keine Veröffentlichung.
...
Die Verwaltung nutzt zusätzlich `local_book_settings` und `local_quote_controls` in derselben privaten SQLite-Datei. Es wird keine weitere Konfiguration benötigt. Nach dem Update eine laufende Anwendung neu starten. Details: [docs/book-management.md](docs/book-management.md).
